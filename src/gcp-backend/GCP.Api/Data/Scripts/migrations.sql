...
DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM __ef_migrations_history WHERE "migration_id" = '20220403101907_EarlyGame') THEN
    CREATE TABLE game (
        id integer GENERATED BY DEFAULT AS IDENTITY,
        name text NOT NULL,
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM __ef_migrations_history WHERE "migration_id" = '20220403101907_EarlyGame') THEN
    INSERT INTO __ef_migrations_history (migration_id, product_version)
    VALUES ('20220403101907_EarlyGame', '7.0.0-preview.2.22153.1');
    END IF;
END $EF$;
COMMIT;
