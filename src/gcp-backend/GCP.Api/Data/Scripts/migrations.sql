...
DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM __ef_migrations_history WHERE "migration_id" = '20220406041439_GameEntity') THEN
    CREATE TABLE game (
        id integer GENERATED BY DEFAULT AS IDENTITY,
        name text NOT NULL,
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM __ef_migrations_history WHERE "migration_id" = '20220406041439_GameEntity') THEN
    INSERT INTO __ef_migrations_history (migration_id, product_version)
    VALUES ('20220406041439_GameEntity', '7.0.0-preview.2.22153.1');
    END IF;
END $EF$;
COMMIT;
...
DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM __ef_migrations_history WHERE "migration_id" = '20220406051044_OwnedGameEntity') THEN
    CREATE TABLE owned_game (
        game_id integer NOT NULL,
        user_id integer NOT NULL,
        CONSTRAINT pk_owned_game PRIMARY KEY (user_id, game_id),
        CONSTRAINT fk_owned_game_asp_net_users_user_id FOREIGN KEY (user_id) REFERENCES "user" (id) ON DELETE CASCADE,
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM __ef_migrations_history WHERE "migration_id" = '20220406051044_OwnedGameEntity') THEN
    INSERT INTO __ef_migrations_history (migration_id, product_version)
    VALUES ('20220406051044_OwnedGameEntity', '7.0.0-preview.2.22153.1');
    END IF;
END $EF$;
COMMIT;
