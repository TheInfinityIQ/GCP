...
public class Seeder : ISeeder
{
	...
	private async Task<User> CreateUserAsync(string email, string password, string displayName, params string[] roles)
	{
	...
	public async Task SeedAsync(CancellationToken cancellationToken = default)
	{
		...
		if (await _context.Users.AnyAsync(cancellationToken) is false)
		{
			...
			var admin = await CreateUserAsync("admin@gcp.com", _options.Value.SharedTestPassword, "TestAdmin", adminRole.Name);
			var moderator = await CreateUserAsync("mod@gcp.com", _options.Value.SharedTestPassword, "TestMod", moderatorRole.Name);
			_ = admin;
			_ = moderator;
		}
